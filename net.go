...
func (m *Memberlist) rawSendMsgPacket(a Address, node *Node, msg []byte) error {
	...
	if m.config.EncryptionEnabled() && m.config.GossipVerifyOutgoing {
		...
		err := encryptPayload(m.encryptionVersion(), primaryKey, msg, packetLabel, cryptBuf)
		if err != nil {
...
func (m *Memberlist) rawSendMsgStream(conn net.Conn, sendBuf *bytes.Buffer, streamLabel string) error {
	...
	if m.config.EncryptionEnabled() && m.config.GossipVerifyOutgoing {
		if chainedBuf == nil {
			cryptBuf, err = m.encryptLocalState(sendBuf.Bytes(), streamLabel)
		} else {
			cryptBuf, err = m.encryptLocalState(chainedBuf.Bytes(), streamLabel)
		}
		if err != nil {
...
func (m *Memberlist) sendUserMsg(a Address, sendBuf []byte) error {
	...
	return m.rawSendMsgStream(conn, bufConn, m.config.Label)
}
...
func (m *Memberlist) sendLocalState(conn net.Conn, join bool, streamLabel string) error {
	...
	// Get the send buffer
	return m.rawSendMsgStream(conn, bufConn, streamLabel)
}
